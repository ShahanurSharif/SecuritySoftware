...
const qrCodes = ref([
    { id: 1, name: 'Main Entrance', location: 'Building A — Front Door', branch: 'Downtown Branch', createdAt: '2026-02-01 09:00:00', status: 'Active' },
    { id: 2, name: 'Parking Level 1', location: 'Building A — Basement P1', branch: 'Downtown Branch', createdAt: '2026-02-01 09:15:00', status: 'Active' },
    { id: 3, name: 'Server Room', location: 'Building B — Floor 3', branch: 'Bay Area Office', createdAt: '2026-02-05 14:30:00', status: 'Active' },
    { id: 4, name: 'Emergency Exit North', location: 'Building A — North Wing', branch: 'Downtown Branch', createdAt: '2026-02-07 11:00:00', status: 'Inactive' }
]);
...
const branchOptions = ref(['Downtown Branch', 'Bay Area Office']);

const emptyQR = { id: null, name: '', location: '', branch: null, status: 'Active' };
...
const filteredQRCodes = computed(() => {
    ...
    return qrCodes.value.filter((item) => {
        return item.name.toLowerCase().includes(q) || item.location.toLowerCase().includes(q) || item.branch.toLowerCase().includes(q);
    });
});
...
const saveQR = () => {
    ...
    if (!qr.value.name.trim() || !qr.value.location.trim() || !qr.value.branch) return;
...
<template>
    ...
    <div class="card">
        ...
        <DataTable :value="filteredQRCodes" :paginator="true" :rows="10" dataKey="id" :rowHover="true" responsiveLayout="scroll">
            ...
            <Column field="branch" header="Branch" sortable style="min-width: 12rem"></Column>
        ...
        <form @submit.prevent="saveQR">
            <div class="flex flex-col gap-6 mt-4">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <Select v-model="qr.branch" :options="branchOptions" placeholder="Select a branch" :invalid="submitted && !qr.branch" />
